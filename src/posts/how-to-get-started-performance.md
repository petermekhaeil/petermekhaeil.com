...
Small note: `moment` was a popular library before it went into [maintainence mode](https://momentjs.com/docs/#/-project-status/) - highly recommend you to start using the other alternative libraries instead.
...
This is a great place to start. Run it on your own websites to get familar with some of the suggestions reported and use them to explore many other techniques in improving performance. The suggestions also link to [web.dev](https://web.dev/) articles that go in details on why these suggestions are listed.
...
The waterfall view is very useful in understanding the load times of each resources being downloaded and which were potientially blocking you from having a faster website. For example - when running a test on my site, we can see 2 resources were downloaded before first render:
